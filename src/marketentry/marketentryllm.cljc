(ns marketentry.marketentryllm
  ...
  the U.S. public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-sam-uei?=" (:has-sam-uei? e)
                        " ein-verified?=" (:ein-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-sam-uei? e))
                              (:has-sam-uei? e))
                          (or (not (:requires-ein? e))
                              (:ein-verified? e)))
                   0.9 0.3)}))
