(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a SAM UEI
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. SAM UEI-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-sam-uei?
                                       ...
                                       Grounded in Japan's SAM entity registration
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(EIN/統一資格/SAM.gov登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- sam-uei-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-sam-uei? true`, INDEPENDENTLY verify
  `:has-sam-uei?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-sam-uei?` ground truth (most JPN public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-sam-uei? e))
                 (not (true? (:has-sam-uei? e))))
        [{:rule :sam-uei-unverified
          :detail (str subject " はSAM UEIを要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- ein-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-ein? true`, INDEPENDENTLY check
  `:ein-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-ein? e))
                 (not (true? (:ein-verified? e))))
        [{:rule :ein-unverified
          :detail (str subject " はEIN確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (sam-uei-unverified-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (ein-unverified-violations request st)
