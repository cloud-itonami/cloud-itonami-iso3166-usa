  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Federal Systems LLC"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Federal Systems LLC" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest sam-uei-unverified-is-held-and-unoverridable
  (testing "missing SAM UEI authorized representative -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:sam-uei-unverified} (-> (store/ledger db) last :basis)))
...
(deftest ein-unverified-is-held-and-unoverridable
  (testing "unverified EIN when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:ein-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
